...
class SaveMiEmpresaRequest extends FormRequest
{
    ...
    {
        $rules = [];
        foreach (MiEmpresaForm::fields() as $name => $field) $rules[$name] = $field[2];
    ...
    {
        $labels = [];
        foreach (MiEmpresaForm::fields() as $name => $field) $labels[$name] = mb_strtolower($field[0]);
