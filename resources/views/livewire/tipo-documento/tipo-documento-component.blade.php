        ...
        <div>
            <div class="col-3 mt-2 mb-2">
                <button wire:click="abrirModal(0)" type="button" data-bs-toggle="modal" data-bs-target="#modalGeneral"
                    class="btn btn-block btn-default btn-flat"><i class="fa fa-plus"></i> Tipo Documento </button>
            </div>
                ...
                <tbody>
                    @foreach ($documento as $doc)
                        <tr>
                            ...
                            <td>{{ $doc->nombre }}</td>
                            <td>
                                <button wire:click="editarDocumento({{ $doc->id }})" type="button"
                                    data-bs-toggle="modal" data-bs-target="#modalGeneral" type="button"
                                    class="btn bg-light btn-xs"><i class="fa fa-pen"></i></button>
                                <button wire:click="borrarDocumento({{ $doc->id }})"
                                    class="btn bg-light btn-xs"><i class="fa fa-trash"></i></button>
                            </td>
                            <td>
                                @if ($doc->defecto)
                                    <small class="badge bg-primary"
                                        wire:click="cambioDefecto({{ $doc->id }})"></i>Activo</small>
                                @else
                                    <small class="badge bg-danger"
                                        wire:click="cambioDefecto({{ $doc->id }})"></i>Desactivado</small>
                                @endif
                            </td>
                            <td>
                                @if ($doc->status)
                                    <small class="badge bg-primary"
                                        wire:click="cambioEstado({{ $doc->id }})"></i>Activo</small>
                                @else
                                    <small class="badge bg-danger"
                                        wire:click="cambioEstado({{ $doc->id }})"></i>Desactivado</small>
                                @endif
                            </td>
                        </tr>
                    @endforeach
