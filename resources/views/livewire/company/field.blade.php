...
<div wire:key="field-{{ $name }}" class="{{ $type === 'textarea' ? 'col-12' : 'col-md-6' }}" data-company-field="{{ $name }}">
    @if($type === 'switch')
    <div class="company-switch p-3 rounded h-100">
        <div class="form-check form-switch mb-0"><input type="checkbox" wire:model="data.{{ $name }}" class="form-check-input @error($name) is-invalid @enderror" id="{{ $name }}" name="{{ $name }}" value="1" {{ $value ? 'checked' : '' }}><label for="{{ $name }}" class="form-check-label">{{ $field[0] }}</label></div>
    </div>
    @else
