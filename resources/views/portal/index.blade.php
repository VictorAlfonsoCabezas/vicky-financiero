        ...
        <section class="panel panel-inverse" id="mis-cuentas">
            ...
            <div class="panel-body">
                <div class="table-responsive"><table class="table align-middle"><thead><tr><th>Código de cuenta</th><th>Tipo de ahorro</th><th>Estado</th></tr></thead>
                    <tbody>@forelse($accounts as $account)<tr><td><a href="{{ route('portal.movements', $account->id) }}">{{ $account->codigo }} ? Ver movimientos</a></td><td>{{ optional($account->tipoAhorros)->name ?: 'Cuenta de ahorro' }}</td><td><span class="badge {{ $account->status ? 'bg-success' : 'bg-secondary' }}">{{ $account->status ? 'Activa' : 'Inactiva' }}</span></td></tr>@empty<tr><td colspan="3" class="text-muted">No tienes cuentas registradas.</td></tr>@endforelse</tbody>
                </table></div>
        ...
        <section class="panel panel-inverse" id="mis-creditos">
            ...
            <div class="panel-body">
                <div class="table-responsive"><table class="table align-middle"><thead><tr><th>Código</th><th>Fecha</th><th class="text-end">Monto solicitado</th><th>Estado</th></tr></thead>
                    <tbody>@forelse($credits as $credit)<tr><td><a href="{{ route('portal.credit', $credit->id) }}">{{ $credit->code }} ? Ver cuotas</a></td><td>{{ $credit->date_created }}</td><td class="text-end">{{ number_format($credit->valor_solicitado, 2) }}</td><td>{{ $credit->status }}</td></tr>@empty<tr><td colspan="4" class="text-muted">No tienes créditos registrados.</td></tr>@endforelse</tbody>
                </table></div>
