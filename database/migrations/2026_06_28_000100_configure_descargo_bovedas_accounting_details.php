...
class ConfigureDescargoBovedasAccountingDetails extends Migration
{
    public function up()
    {
        if (
    ...
    private function configureCompany($companyId)
    {
        ...
        $cash = $this->plan($companyId, '1.1.01.05.01');
        $transitory = $this->plan($companyId, '2.9.90.90.25');
        $expense = $this->plan($companyId, '4.5.07.20');
        ...
        if ($egreso && $expense && $cash) {
            $this->syncOperation($companyId, 'GASINI', $egreso->id, [
                $this->detail($expense->id, true),
                $this->detail($cash->id, false),
            ]);
        }
    }
    ...
    private function syncOperation($companyId, $operationShortName, $conceptoId, array $details)
    {
        ...
        DB::table('config_plan_header')->updateOrInsert(
            [
                ...
                'operaciones_descargo_bovedas_id' => $operation->id,
